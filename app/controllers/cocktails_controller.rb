...
class CocktailsController < ApplicationController
  ...
  def autocomplete
    ...
    @cocktails = if term.present?
                   Drink.where('name LIKE ?', "%#{term}%")
                 else
                   Drink.none
                 end
  ...
  def search
    ...
    uri.query = URI.encode_www_form({ word: params[:word], base: params[:base], alcohol_from: params[:alcohol_from],
  ...
  def result
    ...
    # ランダムに6件を選択し、詳細情報を取得
    @selected_cocktails = @cocktails.sample(6)
